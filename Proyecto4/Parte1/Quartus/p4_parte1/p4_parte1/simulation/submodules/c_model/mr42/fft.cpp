...
void FFT_NAME::fft_floatpt_kernel(double* real, double* imag, double* real_twid, double* imag_twid, int delay, int dw, int num_twids)
{
    ...
    print_floatpt_values((double*)real, (double*)imag, "func_compute_fft: after fft kernel", 0, this_fftpts);
}
...
void FFT_NAME::func_compute_fft(double* real, double* imag)
{
    ...
    int this_stage_delay = (bit_reverse_core ? 1 : (representation == FLOATPT) ? this_fftpts / 4 : this_fftpts / 2);
    int this_dw = in_dw;

    //calc_all_cma_datawidths();

    // for each stage:
    if (bit_reverse_core && representation == FIXEDPT) {
        // bit reverse core
#ifdef DEBUG_FFT
...
{
    return bit_reverse_core && representation == FLOATPT;
}
